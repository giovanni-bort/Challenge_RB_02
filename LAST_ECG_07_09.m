...
clear all;
 curr_dir=pwd;
 if(strncmpi(curr_dir,'C:\Bortolan\Matlab\CINC20',25))
     % Giovanni definitions of directories                                       % Giovanni - 
     fprintf(' *** Giovanni Directory \n');                                      % Giovanni - 
     dir_CINC='C:\Bortolan_dati\Training_WFDB';                                  % Giovanni - Database of CINC20
     dir_CINC='C:\Bortolan_dati\Training_2';                                     % Giovanni - Database of CINC20
     dir_CINC='C:\Bortolan_dati\Training_D';                                     % Giovanni - Database of CINC20
     ris_CINC_ivaylo='C:\Bortolan\Matlab\CINC20\CINC20 Ivaylo\RIS_CINC20';       % Giovanni - Results of all records
     dir_CINC_DL='C:\Bortolan\Matlab\CINC20\CINC20 Ivaylo\ECG_CINC20_DL';        % Giovanni - for Deep Learning
     dir_CINC_DL_01='C:\Bortolan\Matlab\CINC20\CINC20 Ivaylo\ECG_CINC20_DL_01';  % Giovanni - for Deep Learning
 else
     % Ivaylo definitions of directories                                  % Ivaylo -
     fprintf('*** Ivaylo Directory \n');                                  % Ivaylo -
     dir_CINC='C:\Database\Challenge_2020\Data2';
     dir_CINC='C:\Database\Challenge_2020\Data1';                      % Ivaylo - Database of CINC20
     dir_CINC='C:\Database\Challenge_2020\Data3'; 
     dir_CINC='C:\Database\Challenge_2020\Data4'; 
     dir_CINC='C:\Database\Challenge_2020\Data6'; 
     dir_CINC='C:\Database\Challenge_2020\Data5'; 
     
     ris_CINC_ivaylo='C:\Matlab\ivo\Challenge_2020';                      % Ivaylo - Results of all records
end
...
% % ---------------------------------------------------    ONLY FOR GIOVANNI
%  if(contains(curr_dir,'C:\Bortolan\Matlab\CINC20'))   %  ONLY FOR GIOVANNI
%     save_cinc_ivaylo                                  %  ONLY FOR GIOVANNI
%  end                                                  %  ONLY FOR GIOVANNI
%  %--------------------------------------------------     ONLY FOR GIOVANNI

dia_check='QAb';
k_dia_check=0;
for jik=1:numel(STRUCT(j).snomed_code)
    if(strcmp(STRUCT(j).snomed_code{jik},dia_check)),k_dia_check=1; end;
end
fprintf('%4.0f  diagnosis %s : %2.0f\n',j,dia_check,k_dia_check);
if(k_dia_check>0),
    msg = ['Execution terminated for presence of diagnosis: ' dia_check];
pause; %error(msg);
end
