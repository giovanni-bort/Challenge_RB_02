...
I_AVB(j)=0; AFL(j)=0; AF(j)=0; PVC(j)=0; PAC(j)=0; LBBB(j)=0;
...
if std(RR)>35 && std(RR)<46;  PAC(j)=0.7; end
if std(RR)>=46; PAC(j)=1; end
...
% Brady
if PAC(j)~=1 && PVC(j)==0 && 1000/Hz*RRm>1000;
    Brady(j)=1;
else Brady(j)=0;
end
...
%  Q wave in lead aVR wider than 0.5 mm and
...
if P_x<40; P_wave=0; end;% too far
if Iz-P_x<=24; P_wave=0; end % too close
...
I_AVB(j)=0; LPR(j)=0;
if P_wave==1 && Iz-P_x > 120*Hz/1000 && Iz-P_x<=130*Hz/1000
    I_AVB(j)=0.5; LPR(j)=0.5;
end
if P_wave==1 && Iz-P_x > 130*Hz/1000; I_AVB(j)=1; LPR(j)=1;
end
...
if isempty(m)==0
  ...
  %      plot(J/Hz, Lead(J), 'ro', Iz/Hz, Lead(Iz), 'ro')
     ...
     Count(j)=length(Zero1);
     Mean=0; S1=0;
        
    if P_wave==0 && Count(j)>7;
        if std(RR)>35 && std(RR)<46;  AF(j)=0.8; AFL(j)=0.2; end
        if std(RR)>=46; AF(j)=0.9; AFL(j)=0.1; end
        if std(RR)<=35;  AF(j)=0.6; AFL(j)=0.4; end
        figure(1); hold on
        plot(Zero1/Hz, Lead(Zero1), 'go')
    end
end
...
X=Wing;    n=50; [Y]=smooth(X,n);		Wing=Y;

[Mi2,mi2]=max(Wing(50:length(Wing)));
Tp=J+s+50+mi2;
figure(2)
subplot(3,3,5);
hold on
plot(x1,V2_mean, x1(J+s:length(Lead)-s), Wing, 'r',  Tp/Hz, V2_mean(Tp), 'ro')%,
...
Iz=Izsave;
J=Jsave;
I_mean=I_mean-I_mean(Iz);
III_mean=III_mean-III_mean(Iz);

ad1=0.0001; ad2=0.0001; ad3=0.0001; ad4=0.0001; ad5=0.0001; ad6=0.0001;
for i=Iz:J
    if I_mean(i)>0; ad1=I_mean(i)+ad1;
    else ad2=abs(I_mean(i))+ad2;
    end
end
    
for i=Iz:J
    if III_mean(i)>0; ad3=III_mean(i)+ad3;
    else ad4=abs(III_mean(i))+ad4;
    end
end

for i=Iz:J
    if II_mean(i)>0; ad5=II_mean(i)+ad5;
    else ad6=abs(II_mean(i))+ad6;
    end
end

if (RAD(j)==1 || LAD(j)==1) && ad1/ad2 > 2 && ad4/ad3 > 2 && ad6/ad5>2
    LAnFB(j)=0.4;
    if LAnFB(j)>0 && ad1/ad2> 150; LAnFB(j)=LAnFB(j)+.2; end
    if LAnFB(j)>0 && ad4/ad3 >40; LAnFB(j)=LAnFB(j)+.2; end
    if LAnFB(j)>0 && ad6/ad5 >20; LAnFB(j)=LAnFB(j)+.2; end
else LAnFB(j)=0;
end


% PR - sharp slope
slope1(1:31)=abs(aVF_mean(Iz-15:Iz+15)- aVF_mean(Iz-14:Iz+16));
[Max1,m1]=max(slope1);
slope2(1:31)=abs(aVR_mean(Iz-15:Iz+15)- aVR_mean(Iz-14:Iz+16));
[Max2,m2]=max(slope2);
figure(3)
subplot(1,2,2)
plot(x1, aVF_mean, Iz/Hz, aVF_mean(Iz), 'ro',(Iz+m-1)/Hz, aVF_mean(Iz+m-1),'ro')
Max=max(Max1,Max2);
if Max>0.13 && P_wave==0;
    PR(j)=0.8; AF(j)=0;
    if RAD(j)==1 || LAD(j)==1; PR(j)=1; end
else PR(j)=0;
end
...
TEXT4=['LQRSV=' num2str(LQRSV(j)), '  TAb=', num2str(TAb(j)), '  TInv=' , num2str(TInv(j)),...
    '  SB=', num2str(SB(j)), '  QAb=', num2str(QAb(j)), ' VEB=', num2str(VEB(j))];
...
fprintf('%s\n%s\n',T8,T9);
fprintf('%3.0f  ',my_label); fprintf(' ==> %6.0f',27-sum(my_label==1)-sum(my_label==0));
fprintf('\n');

% fprintf('%5.2f  ',my_label); fprintf('\n');
